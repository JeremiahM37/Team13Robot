...
async function startGreeter(keyboard) {
    const result = await apiCall('/api/greeter/start', { keyboard: keyboard });
    if (result.success) {
        autoStatusEl.textContent = `Greeter (${keyboard ? 'keyboard' : 'voice'}) - running`;
        autoStatusEl.className = 'auto-status auto-active';
    }
}

async function stopAutonomous() {
    await apiCall('/api/wall-follow/stop', {});
    await apiCall('/api/greeter/stop', {});
    autoStatusEl.textContent = 'Idle';
...
document.getElementById('greeter-keyboard-btn').addEventListener('click', () => startGreeter(true));
document.getElementById('auto-stop-btn').addEventListener('click', stopAutonomous);
