...
@app.route('/api/wall-follow/start', methods=['POST'])
def wall_follow_start():
    """Start wall following mode."""
    global wall_follower
    # Stop greeter if running
    if greeter and greeter.active:
        greeter.stop()
    # Stop existing wall follower
    if wall_follower and wall_follower.active:
        wall_follower.stop()
    ...
    if lidar:
        lidar.add_scan_callback(wall_follower.update_scan)
    wall_follower.start()
    return jsonify({'success': True, 'side': side, 'speed': speed})


@app.route('/api/wall-follow/stop', methods=['POST'])
def wall_follow_stop():
    """Stop wall following mode."""
    if wall_follower:
        if lidar:
            lidar.remove_scan_callback(wall_follower.update_scan)
        wall_follower.stop()
...
@app.route('/api/wall-follow/status', methods=['GET'])
def wall_follow_status():
    """Get wall follower status."""
    if wall_follower:
        return jsonify(wall_follower.get_status())
    return jsonify({'active': False, 'state': 'IDLE'})


@app.route('/api/greeter/start', methods=['POST'])
def greeter_start():
    """Start greeter mode."""
    global greeter
    ...
    if wall_follower and wall_follower.active:
        if lidar:
            lidar.remove_scan_callback(wall_follower.update_scan)
        wall_follower.stop()
    # Stop existing greeter
    if greeter and greeter.active:
        greeter.stop()

    data = request.get_json() or {}
    use_keyboard = data.get('keyboard', False)

    greeter = RobotGreeter(robot, use_keyboard=use_keyboard)
    if lidar:
        lidar.add_scan_callback(greeter.update_scan)
    greeter.start()
    return jsonify({'success': True, 'keyboard': use_keyboard})


@app.route('/api/greeter/stop', methods=['POST'])
def greeter_stop():
    """Stop greeter mode."""
    if greeter:
        if lidar:
...
@app.route('/api/greeter/status', methods=['GET'])
def greeter_status():
    """Get greeter status."""
    if greeter:
        return jsonify(greeter.get_status())
    return jsonify({'active': False, 'state': 'IDLE'})
